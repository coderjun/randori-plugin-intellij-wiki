...
7. Above the constructor for IndexMediator, you will add a new public variable named message of type 
   ...
   However, if you recall, you earlier learned  that the goal of behaviors was to decorate DOM elements and provide an 
   API. You currently have a behavior defined on the div with an id of messageWindow. That behavior indicates 
   that it should be decorated by MessageWindow. Therefore, instead of receiving a DOM fragment in the
...
8.  Notice that the class created already has an override the protected onRegister() method of the mediator:
   ...
   Mediators are a type of behavior and also have an onRegister() method. Now a word on timing. Randori 
...
13. Green the green play button to launch the application. 
    ...
    Your mediator is now interacting with your behavior instead of on the DOM directly.  It is important to not that the mediator can call methods on a behavior which will interact with the DOM. This is the key to the way Randori works and builds.
