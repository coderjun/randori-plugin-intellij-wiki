...
5. Inside of the selector, add a style for –randori-mediator: mediators.IndexMediator;
   ...
   Randori introduces some specific styles of its own, to allow you to define behaviors in CSS.  It is possible to mix these 
   with traditional regular CSS style sheets, but it is recommended to keep them separate in their own  Behavior Style 
   Sheet is a better practice.

   In this case you are indicating that anything with the CSS class simpleApp will usean instance of the IndexMediator class as a mediator.
